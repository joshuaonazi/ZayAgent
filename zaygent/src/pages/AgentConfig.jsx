          ...
          <div style={cardStyle}>
            ...
            <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
              {Object.entries(enabledChains).map(([chain, enabled]) => {
                const chainColor = chains[chain]?.color || COLORS.teal;

                return (
                <div key={chain} onClick={() => setEnabledChains(p => ({ ...p, [chain]: !p[chain] }))}
                  style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: enabled ? chainColor + "11" : COLORS.bg, border: `1px solid ${enabled ? chainColor + "44" : COLORS.border}`, borderRadius: 7, padding: "10px 12px", cursor: "pointer", transition: "all 0.2s" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: enabled ? chainColor : COLORS.textMuted }} />
                    <span style={{ fontSize: 12, color: enabled ? COLORS.textPrimary : COLORS.textSecondary, fontWeight: 500 }}>{chain}</span>
                  </div>
                  <span style={{ fontSize: 10, color: enabled ? chainColor : COLORS.textMuted, fontWeight: 700 }}>
