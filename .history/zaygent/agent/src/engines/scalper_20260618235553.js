...
const elapsed     = (Date.now() - new Date(position.openedAt).getTime()) / 1000 / 60; // minutes open
const drift       = elapsed * 0.008; // 0.8% drift per minute upward
const volatility  = (Math.random() * 0.1) - 0.05; // ±5% random noise
const currentPrice = position.entryPrice * (1 + drift + volatility);    const result = evaluate(position, currentPrice);
